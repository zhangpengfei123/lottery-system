...
```bash
curl --request GET \
  --url "http://localhost:8091/api/v1/raffle/strategy/strategy_armory?strategyId=100001"
```
...
```bash
curl --request POST \
  --url "http://localhost:8091/api/v1/raffle/strategy/query_raffle_award_list" \
  --header "content-type: application/json" \
  --data "{\"strategyId\":100001}"
```
...
curl --request POST \
  ...
  --header "content-type: application/json" \
  --data "{\"userId\":\"zpf\",\"activityId\":100301}"
```
...
1. `big-market-app/src/main/java/cn/bugstack/Application.java`
   先看应用如何启动，以及 Spring Boot 如何扫描和装配各模块。

2. `big-market-trigger/src/main/java/cn/bugstack/trigger/http`
   从 HTTP 接口入手，理解请求是如何进入系统的。

3. `big-market-domain/src/main/java/cn/bugstack/domain/strategy`
   阅读抽奖策略相关代码，重点看责任链、规则树和库存扣减逻辑。

4. `big-market-domain/src/main/java/cn/bugstack/domain/activity`
   阅读活动参与、额度校验、活动 SKU 库存相关流程。

5. `big-market-domain/src/main/java/cn/bugstack/domain/award`
   阅读中奖记录写入和发奖任务生成。

6. `big-market-infrastructure/src/main/java/cn/bugstack/infrastructure`
   理解仓储接口如何通过 MyBatis、Redis、RabbitMQ 落地。

7. `big-market-trigger/src/main/java/cn/bugstack/trigger/job`
   阅读定时任务，理解库存落库和消息补偿机制。
